...
##
### 5. Discussion
######  앞서 살펴본대로, CNN  일련의 역사와 Alexnet이 어떻게 발전했는지를 쭉 돌이켜 보았을 때, LeNet과 Alexnet 모두 주어진 imagedataset(MNIST, Imagenet)을 어떻게 더 정확하게 분류해 낼 수 있을까에서 출발해 계속 발전했다고 볼 수 있을것 같습니다. 이러한 관점에서 보았을 때, deep-learning 문제 해결에 있어, '어떤 모델을 써볼까?'라는 생각보다 '어떤 데이터를 사용해서 무엇을 분류해낼까?'를 더 우선적으로, 중심에 놓고 생각해야할 것 같습니다.
